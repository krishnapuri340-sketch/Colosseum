...
const focusBorder = "rgba(249,115,22,0.6)";
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative" }}>
        ...
        />
        {(icon || toggle) && (
          <div style={{ position: "absolute", right: "0.85rem", top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.3)", display: "flex" }}>
            {toggle ?? icon}
          </div>
        )}
      </div>
    ...
    <>
      <div style={{ marginBottom: "1.75rem" }}>
        ...
        <p style={{ margin: "0.5rem 0 0", color: "rgba(255,255,255,0.45)", fontSize: "0.875rem" }}>
          ...
          <button onClick={onSwitch} style={{ background: "none", border: "none", color: "#f97316", fontWeight: 700, cursor: "pointer", fontSize: "0.875rem", padding: 0 }}>
            Create Account
          </button>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        {error && <div style={{ padding: "0.7rem 1rem", background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.35)", borderRadius: 8, color: "#f87171", fontSize: "0.85rem" }}>{error}</div>}
        <Field label="Email" type="email" value={email} onChange={setEmail} placeholder="you@example.com" icon={<Mail style={{ width: 16, height: 16 }} />} />
        <Field
        ...
        />
        <button type="submit" disabled={loading} style={{ padding: "0.9rem", background: loading ? "rgba(249,115,22,0.4)" : "#f97316", border: "none", borderRadius: 10, color: "#fff", fontSize: "0.95rem", fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", cursor: loading ? "not-allowed" : "pointer", marginTop: "0.25rem", boxShadow: loading ? "none" : "0 4px 20px rgba(249,115,22,0.4)", transition: "opacity 0.2s" }}
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
        {error && <div style={{ padding: "0.7rem 1rem", background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.35)", borderRadius: 8, color: "#f87171", fontSize: "0.85rem" }}>{error}</div>}
        <Field label="Full Name" type="text" value={name} onChange={setName} placeholder="Rohit Sharma" icon={<User style={{ width: 16, height: 16 }} />} />
        <Field label="Email" type="email" value={email} onChange={setEmail} placeholder="you@example.com" icon={<Mail style={{ width: 16, height: 16 }} />} />
        <Field
...
export default function AuthPages({ mode }: { mode: "login" | "register" }) {
  ...
  return (
    <div style={{
      ...
      {/* Card — stays mounted, only content fades on switch */}
      <div style={{
        position: "relative",
        zIndex: 2,
        width: "100%",
        ...
        <div style={{ marginBottom: "2.25rem" }}>
          <div style={{ fontWeight: 900, fontSize: "1.2rem", color: "#fff", letterSpacing: "-0.02em", lineHeight: 1 }}>Colosseum</div>
          <div style={{ fontSize: "0.65rem", color: "rgba(255,255,255,0.4)", letterSpacing: "0.12em", textTransform: "uppercase" }}>IPL Fantasy</div>
        </div>
